...
const ExpenseCategoryLine = ({
  budgetId,
  expenseCategory,
  isEditable,
}: ExpenseCategoryLineProps) => {
  const router = useRouter();
  const currencySymbol = useCurrencySymbol();
  const totalExpense = expenseCategory.Expenses.reduce(
    (acc, expense) => acc + expense.Amount,
    0,
  );
  const totalUsage =
    totalExpense >= expenseCategory.Amount
      ? 1.0
      : (totalExpense * 1.0) / expenseCategory.Amount;
  const usageIcon =
    totalUsage === 0.0
      ? "circle-outline"
