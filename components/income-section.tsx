...
const IncomeSection = ({ currentBudget, isActive }: IncomeSectionProps) => {
  ...
  const totalIncome = currentBudget?.Incomes.reduce(
    (acc, income) => acc + income.Amount,
    0,
  );
  const sheetRef = React.useRef<BottomSheetModal>(null);
  const [incomeToDelete, setIncomeToDelete] = React.useState<Income | null>(
    null,
  );
    ...
    <>
      ...
      >
        <Card.Title
          ...
          subtitle={
            currentBudget !== null &&
            `Total: ${currencySymbol} ${totalIncome?.toLocaleString("en-US", numberOption)}`
          }
